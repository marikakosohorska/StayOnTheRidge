...
function example1(sym::Bool)
    ...
    if sym
        ...
        return Config_FD(f_fd, n, min_coords, γ, ϵ, [[0,1],[0,1]])
    end
end
...
function example3(sym::Bool)
    ...
    if sym
        ...
        return Config_sym(f, x, n, min_coords, γ, ϵ, [[-1,1],[-1,1]])
    else
        function f_fd(x)
        ...
        return Config_FD(f_fd, n, min_coords, γ, ϵ, [[-1,1],[-1,1]])
    end
end
...
function example4(sym::Bool)
    ...
    if sym
        ...
        q = x[1]+x[4]+1
        f = p/q
        return Config_sym(f, x, n, min_coords, γ, ϵ, [fill([0,1], 4)...])
    else
        p_fd(x) = x[1]^2+x[2]^2-x[3]^2-x[4]^2
        q_fd(x) = x[1]+x[4]+1
        f_fd(x) = p_fd(x)/q_fd(x)
        return Config_FD(f_fd, n, min_coords, γ, ϵ, [fill([0,1], 4)...])
    end
end
...
function example7(sym::Bool)
    ...
    if sym
        ...
        return Config_sym(f, x, n, min_coords, γ, ϵ, [fill([-1,1], 6)...])
    else
        f_fd(x) = sum(x[i]+x[3+i] for i in 1:3) - prod((x[i] - x[3+i]) for i in 1:3)
        return Config_FD(f_fd, n, min_coords, γ, ϵ, [fill([-1,1], 6)...])
    end
end
...
function example11(sym::Bool)
    ...
    if sym
        ...
        return Config_sym(f, x, n, min_coords, γ, ϵ, [[-1,1],[-2*pi,2*pi]])
    else
        f_fd(x) = 0.2*x[1]*x[2]-cos(x[2])
        return Config_FD(f_fd, n, min_coords, γ, ϵ, [[-1,1],[-2*pi,2*pi]])
    end
end
